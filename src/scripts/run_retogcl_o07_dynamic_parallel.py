...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="o07_dynamic")
...
def write_status(path: Path, total: int, queue: list, running: dict, completed: int, skipped: int, failures: list) -> None:
    value = {
        ...
        "failures": [
            {"config_id": config_id, "dataset": dataset, "exit_code": code, "log": str(log)}
...
def main() -> None:
    ...
    if len(configs) != 20:
        raise ValueError(f"阶段 {args.stage} 配置数不是 20")
    ...
    failures = []
    completed = 0
    progress = tqdm(total=total, initial=skipped, desc=f"{args.tag} 动态调参", unit="任务")
    while queue or running:
        ...
        for device in args.devices:
            for slot in range(args.jobs_per_device):
                ...
                command = [
                    sys.executable,
                    str(PROJECT_ROOT / "src/scripts/run_retogcl_o07_dynamic_five_fold.py"),
                ...
                process = subprocess.Popen(command, cwd=PROJECT_ROOT, stdout=handle, stderr=subprocess.STDOUT, env=environment)
