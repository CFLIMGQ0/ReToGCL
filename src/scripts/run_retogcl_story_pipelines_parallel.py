"""在多张 GPU 上并行运行8套故事 Pipeline×6个目标数据集。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="story_host202")
...
def write_status(
    ...
    value = {
        ...
        "failures": [
            {"pipeline": pipeline, "dataset": dataset, "exit_code": code, "log": str(log)}
            for pipeline, dataset, code, log in failures
        ],
    }
...
def main() -> None:
    ...
    all_tasks = [
        ...
        if (pipeline_index + dataset_index) % args.shard_count in selected
    ]
    output_dir = (
    ...
    queue = []
    skipped = 0
    for pipeline, dataset in all_tasks:
        result_path = output_dir / "jobs" / f"{pipeline}__{dataset}.json"
    ...
    completed = 0
    progress = tqdm(
        total=total, initial=skipped, desc=f"{args.tag} 故事Pipeline", unit="任务"
    )
        ...
        for device in args.devices:
            for slot in range(args.jobs_per_device):
                ...
                if memory.get(device, 0) < args.min_free_memory_mib:
                    break
                pipeline, dataset = queue.pop(0)
                log_path = log_dir / f"{pipeline}__{dataset}_gpu{device}_slot{slot}.log"
                handle = log_path.open("w", encoding="utf-8")
                command = [
                    sys.executable,
                    str(PROJECT_ROOT / "src/scripts/run_retogcl_story_pipeline_five_fold.py"),
                    "--pipeline", pipeline,
                    "--dataset", dataset,
                ...
                running[key] = (process, pipeline, dataset, log_path, handle)

        finished = []
        for key, (process, pipeline, dataset, log_path, handle) in running.items():
            code = process.poll()
            if code is None:
                continue
            handle.close()
            if code:
                failures.append((pipeline, dataset, code, log_path))
            else:
                completed += 1
            finished.append(key)
            progress.update(1)
    ...
    if failures:
        raise SystemExit("存在失败任务：\n" + "\n".join(
            f"- {pipeline}/{dataset}: exit={code}, log={log}"
            for pipeline, dataset, code, log in failures
        ))
