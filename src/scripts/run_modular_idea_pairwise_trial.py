"""运行一个固定模块化 ABC 参数的跨模块双 idea、单数据集五折任务。"""
...
from src.models.modular_pairwise_research_ideas import (
    build_modular_pairwise_research_idea_model,
)
...
def main() -> None:
    ...
    if first_module == second_module:
        raise RuntimeError(f"同模块 idea 不允许组合：{first_idea}+{second_idea}")
    if task.get("first_module") != first_module or task.get("second_module") != second_module:
    ...
    print(
        f"跨模块组合任务：{args.task_id}，模块：{first_module}+{second_module}，设备：{device}"
    )

    graphs, in_dim, classes = load_graphs(data_root, dataset)
    set_seed(args.seed)
    model = build_modular_pairwise_research_idea_model(
        first_idea,
        second_idea,
        in_dim,
        args.hidden_dim,
    ...
    print(f"跨模块两两组合任务完成：{result_path}")
