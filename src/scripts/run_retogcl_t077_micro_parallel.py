...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="t077_micro")
...
def main() -> None:
    ...
    configs = [
        entry for entry in manifest["configs"] if entry["stage"] == args.stage
    ]
    if len(configs) != 20:
    ...
    while queue or running:
        ...
        for device in args.devices:
            for slot in range(args.jobs_per_device):
                ...
                log_path = log_dir / (
                    f"{config_id}__{dataset}_gpu{device}_slot{slot}.log"
                )
                handle = log_path.open("w", encoding="utf-8")
                command = [
                    sys.executable,
                    str(PROJECT_ROOT / "src/scripts/run_retogcl_t077_micro_five_fold.py"),
                ...
                running[key] = (
                    process, config_id, dataset, log_path, handle
                )
