"""在多张 GPU 上运行 10 个 O07 参数版本×6个数据集。"""
...
VARIANTS = tuple(f"P{index:02d}" for index in range(1, 11))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", default="o07_parameters_host202")
...
def write_status(
    ...
    value = {
        ...
        "failures": [
            {"variant": variant, "dataset": dataset, "exit_code": code, "log": str(log)}
            for variant, dataset, code, log in failures
        ],
    }
...
def main() -> None:
    ...
    all_tasks = [
        ...
        if (variant_index + dataset_index) % args.shard_count in selected
    ]
    output_dir = args.output_dir if args.output_dir.is_absolute() else (PROJECT_ROOT / args.output_dir).resolve()
    queue = []
    skipped = 0
    for variant, dataset in all_tasks:
        result_path = output_dir / "jobs" / f"{variant}__{dataset}.json"
        if result_path.exists() and not args.force:
            try:
                result = json.loads(result_path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                result = {}
            if result.get("status") == "completed" and result.get("protocol") == "retogcl_o07_parameter_variant_v1":
                skipped += 1
                continue
        queue.append((variant, dataset))
    ...
    failures = []
    completed = 0
    progress = tqdm(total=total, initial=skipped, desc=f"{args.tag} O07调参", unit="任务")
    ...
    while queue or running:
        ...
        for device in args.devices:
            for slot in range(args.jobs_per_device):
                ...
                if memory.get(device, 0) < args.min_free_memory_mib:
                    break
                variant, dataset = queue.pop(0)
                log_path = log_dir / f"{variant}__{dataset}_gpu{device}_slot{slot}.log"
                handle = log_path.open("w", encoding="utf-8")
                command = [
                    sys.executable,
                    str(PROJECT_ROOT / "src/scripts/run_retogcl_o07_parameter_five_fold.py"),
                    "--variant", variant,
                    "--dataset", dataset,
                ...
                running[key] = (process, variant, dataset, log_path, handle)

        finished = []
        for key, (process, variant, dataset, log_path, handle) in running.items():
            code = process.poll()
            if code is None:
                continue
            handle.close()
            if code:
                failures.append((variant, dataset, code, log_path))
            else:
                completed += 1
            finished.append(key)
            progress.update(1)
        for key in finished:
            del running[key]
        progress.set_postfix(等待=len(queue), 运行=len(running), 完成=completed, 跳过=skipped, 失败=len(failures))
    ...
    if failures:
        raise SystemExit("存在失败任务：\n" + "\n".join(
            f"- {variant}/{dataset}: exit={code}, log={log}"
            for variant, dataset, code, log in failures
        ))
