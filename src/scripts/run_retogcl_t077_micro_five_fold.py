#!/usr/bin/env python3
"""运行一个 T077 微调配置，并在指定数据集执行五折线性评测。"""
...
from src.models.retogcl_t077_micro_tuning import (
    ...
    build_retogcl_t077_micro_tuning,
)
...
def main() -> None:
    ...
    configuration = T077MicroConfiguration(**entry["configuration"])
    ...
    set_seed(args.seed)
    model = build_retogcl_t077_micro_tuning(
        configuration,
        in_dim,
        args.hidden_dim,
        args.layers,
    ...
    result = {
        ...
        "status": "completed",
        "model": f"ReToGCL-T077-{args.config_id}",
        "config_id": args.config_id,
        "stage": entry["stage"],
        "focus": entry["focus"],
