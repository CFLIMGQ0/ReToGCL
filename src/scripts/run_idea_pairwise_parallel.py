...
WORKER = PROJECT_ROOT / "src/scripts/run_idea_pairwise_trial.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prefilter-existing", action="store_true")
...
def write_status(path: Path, *, total: int, queue: list, running: dict, completed: int, skipped: int, failures: list) -> None:
    payload = {
        ...
        "stage": "pairwise_fixed_global_ABC",
        "total": total,
        "queued": len(queue),
...
def main() -> None:
    args = parse_args()
    if args.jobs_per_device != 3:
        raise SystemExit("本轮要求每张 GPU 固定三个 Lim 任务，请使用 --jobs-per-device 3")
