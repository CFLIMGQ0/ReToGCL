"""在多张 GPU 上并行运行 ReToGCL-Linked 的18数据集实验。"""
...
def write_status(path: Path, total: int, queue: list[str], running: dict, completed: int, failures: list) -> None:
    value = {
...
def main() -> None:
    ...
    queue = [
        dataset for index, dataset in enumerate(DATASETS)
    ...
    failures = []
    completed = 0
    progress = tqdm(total=total, desc=f"{args.tag} ReToGCL-Linked", unit="数据集")
    ...
    while queue or running:
        ...
        for device in args.devices:
            for slot in range(args.jobs_per_device):
                ...
                if memory.get(device, 0) < args.min_free_memory_mib:
                    break
                dataset = queue.pop(0)
                log_path = log_dir / f"{dataset}_gpu{device}_slot{slot}.log"
                handle = log_path.open("w", encoding="utf-8")
                command = [
                    sys.executable,
                    str(PROJECT_ROOT / "src/scripts/run_retogcl_linked_five_fold.py"),
                    "--dataset", dataset,
        ...
        write_status(status_path, total, queue, running, completed, failures)
        if queue or running:
            time.sleep(max(1, args.poll_seconds))
    progress.close()
    if failures:
        raise SystemExit("存在失败任务：\n" + "\n".join(
            f"- {dataset}: exit={code}, log={log}" for dataset, code, log in failures
        ))
