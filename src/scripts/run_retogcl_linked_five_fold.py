"""运行独立的 ReToGCL A→B→C 链接式 Pipeline，并进行五折线性评测。"""
...
from src.models.retogcl_linked_pipeline import build_retogcl_linked_pipeline
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/retogcl_linked_pipeline_18"))
...
def main() -> None:
    ...
    result_path = args.output_dir / "jobs" / f"{MODEL_NAME}__{args.dataset}.json"
    ...
    set_seed(args.seed)
    model = build_retogcl_linked_pipeline(
        in_dim, args.hidden_dim, args.layers, classes, IDEA_PARAMETERS
    ).to(device)
    checkpoint = args.output_dir / "checkpoints" / f"{args.dataset}.pt"
    log_path = args.output_dir / "logs" / f"{args.dataset}.jsonl"
    ...
    if checkpoint.exists() and not args.force:
        ...
        with log_path.open("w", encoding="utf-8") as log_file:
            progress = tqdm(
                range(1, args.epochs + 1), desc=f"{MODEL_NAME}/{args.dataset}", unit="轮"
            )
            for epoch in progress:
                ...
                for batch in loader:
                    ...
                    if not torch.isfinite(loss):
                        raise RuntimeError(f"{args.dataset}/epoch={epoch} 损失非有限")
                    loss.backward()
                    gradient = torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
                    if not torch.isfinite(gradient):
                        raise RuntimeError(f"{args.dataset}/epoch={epoch} 梯度非有限")
                    optimizer.step()
                ...
                progress.set_postfix(
                    ...
                    link=f"{record['link_strength']:.3f}",
                )
    ...
    metric_result = evaluate_embeddings(
        model, graphs, embeddings, labels, args.dataset, device, args
    )
    result = {
        ...
        "pipeline": [
            "augmentation_with_trace",
            "trace_conditioned_soft_cleaning",
            "reliability_conditioned_topology_projection",
            "balanced_contrastive_objective",
        ],
