...
from src.models.retogcl_single_point_optimizations import (
    ...
    OPTIMIZATIONS,
    build_retogcl_single_point_optimization,
)
...
O07_PROTOCOL = "retogcl_single_point_optimization_o07_temperature_006_v2"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--optimization", choices=OPTIMIZATIONS, required=True)
...
def main() -> None:
    ...
    spec = OPTIMIZATIONS[args.optimization]
    protocol = O07_PROTOCOL if args.optimization == "O07" else PROTOCOL
    result_path = (
        args.output_dir / "jobs" / f"{args.optimization}__{args.dataset}.json"
    )
    ...
    set_seed(args.seed)
    model = build_retogcl_single_point_optimization(
        args.optimization, in_dim, args.hidden_dim, args.layers, classes,
        IDEA_PARAMETERS,
    ).to(device)
    checkpoint = (
        args.output_dir / "checkpoints" / args.optimization / f"{args.dataset}.pt"
    )
    log_path = (
        args.output_dir / "logs" / args.optimization / f"{args.dataset}.jsonl"
    )
    expected = {
        "protocol": protocol,
    ...
    if args.optimization == "O07":
        expected["contrastive_temperature"] = O07_DEFAULT_TEMPERATURE
    ...
    if checkpoint.exists() and not args.force:
        ...
        with log_path.open("w", encoding="utf-8") as log_file:
            progress = tqdm(
                ...
                desc=f"{args.optimization}/{args.dataset}", unit="轮",
            )
            for epoch in progress:
                ...
                for batch in loader:
                    ...
                    if not torch.isfinite(loss):
                        raise RuntimeError(
                            f"{args.optimization}/{args.dataset}/epoch={epoch} 损失非有限"
                        )
                    loss.backward()
                    gradient = torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
                    if not torch.isfinite(gradient):
                        raise RuntimeError(
                            f"{args.optimization}/{args.dataset}/epoch={epoch} 梯度非有限"
                        )
                    optimizer.step()
    ...
    result = {
        ...
        "status": "completed",
        "model": f"ReToGCL-{args.optimization}",
        "optimization": args.optimization,
        "optimization_name": spec.name,
        "changed_component": spec.component,
        "single_change": spec.change,
        "dataset": args.dataset,
        "dataset_protocol": dataset_protocol(args.dataset),
        "protocol": protocol,
        "base_model": "ReToGCL_D7-I10_D7-I01_D10-I04",
